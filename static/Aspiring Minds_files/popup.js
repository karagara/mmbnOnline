...
function UpdateTimer() {
		...
		if(curMinutes > 0){
			...
			if (curHour > 0 ){
			 ...
			 curMinutes = 59
				...
				timerStop();
				//if(showAlert == 1 && document.frmMain.actionButton.value!="ShowModuleDescription" && document.frmMain.actionButton.value!="getFirstQuestion")
   ...
   timerID = setTimeout("UpdateTimer()", timeInterval);
}

function moduleTimeOutSubmit(){
        $("#timeOutMsg").html('<div class="msg-loader">Please wait... <img src="images/ajax-loading.gif"></div>');
	document.getElementById(timerDispayDiv).innerHTML =  "";
	var queryString = document.dummy.queryString.value;
	doDummySubmit(queryString);
	//doDummySubmit();
...
function checkLanguage ()
{
	...
	if( languagecode == 2 )
	{
		$("#SelectedLanguageMsg").html ("You have selected <strong>Java</strong> as your preferred programming language. Press 'OK' to proceed, or press 'Cancel' to change your selection. " );

	}	
	if( languagecode == 3 )
	{
		$("#SelectedLanguageMsg").html ("You have selected <strong>C++</strong> as your preferred programming language. Press 'OK' to proceed, or press 'Cancel' to change your selection. " );
	}
...
function disableNavigation(){
	...
	if(document.getElementById('tdNext')){
		...
		document.getElementById('tdNext').innerHTML='<span><span class="ico"></span>Next</span>';
		
	}
	
	if(document.getElementById('tdNextStartTest')){
		$("#tdNextStartTest").removeClass("continue btn wauto").addClass("continue disabledbtn btn wauto");
		$('#tdNextStartTest').attr('onclick','').unbind('click');
		document.getElementById('tdNextStartTest').innerHTML='<span><span class="ico"></span>Start Test</span>';
	...
	if(document.getElementById('tdNextQuestion')){
		...
		document.getElementById('tdNextQuestion').innerHTML='<span><span class="ico"></span>Submit Answer</span>';
		
	}
	
	if(document.getElementById('tdExitModule')){
	    //$("#tdExitModule").removeClass("module_exit_v").addClass("module_exit_a");
	    document.getElementById('tdExitModule').innerHTML='<a><span class="ico"></span>Module Exit</a>';
	}
	
	if(document.getElementById('tdExitTest')){
		//$("#tdExitTest").removeClass("test_exit_v").addClass("test_exit_a");
		document.getElementById('tdExitTest').innerHTML='<a><span class="ico"></span>Test Exit</a>';
	}
	
	if(document.getElementById('tdShowHelp')){
	    //$("#tdShowHelp").removeClass("help_v").addClass("help_a");
	    document.getElementById('tdShowHelp').innerHTML='<a><span class="ico"></span>Help</a>';
	}
	...
	if(document.getElementById('tdNextModule')){
	   ...
	   document.getElementById('tdNextModule').innerHTML='<span><span class="ico"></span>Proceed</span>';
	   // $("#tdConfirmAnswer").removeClass("confirm btn").addClass("confirm disabledbtn btn");
	//document.getElementById('tdConfirmAnswer').innerHTML='<span><span class="ico"></span>Confirm Answer</span>';
...
function exitOK(act) {
	...
	$('#cancel').remove();
        id =   act+"Msg";
        $("#"+id).html('<div class="msg-loader">Please wait... <img src="images/ajax-loading.gif"></div>');
	document.frmMain.actionButton.value=act;
	document.frmMain.submit();
	return true;	
}
...
function doDummySubmit(queryString)
{
	...
	if(typeof document.frmMain.actionButton== 'undefined'){
	   ...
	   //document.dummy.action='doTest.php'; 
	   document.dummy.submit();
	}else if(document.frmMain.actionButton.value=="ShowModuleDescription"){
...
function doTabsDisable() {
	...
	if(finalLastSid == sID) {	
		...
		$('#greyImage').html('Next Question');	
	}
	//$('#greyImage').attr('onclick','').unbind('click');
	//alert("hi1");
	$('#saveDiv').removeClass('btnsm btns bsave btns-t customBtn').addClass('btnsm btns bsave btns-t customBtn disabledbtn2');
	$('#saveCode').attr('onclick','').unbind('click');
	
	$('#compileDiv').removeClass('btnsm btns bcompile btns-t customBtn').addClass('btnsm btns bcompile btns-t customBtn disabledbtn2');
	$('#compile').attr('onclick','').unbind('click');
...
function doTabsEnable() {
	//alert("hi1");
	$('#saveDiv').removeClass('btnsm btns bsave btns-t customBtn disabledbtn2').addClass('btnsm btns bsave btns-t customBtn');
	  $('#saveCode').css('color','white');
	  $('#saveCode').html("<span><span class='ico'></span>Save</span>");
	  $("#saveDiv").html("<a class='compilerButtonColor' title='Save Code' id='saveCode' onclick='javascript: return saveCode(editor.getValue());' href='#' ><span><span class='ico'></span>Save</span></a>"); 
	
	$('#compileDiv').removeClass('btnsm btns disabledbtn2 bcompile btns-t').addClass('btnsm btns bcompile btns-t');
	$("#compileDiv").html('<a class="compilerButtonColor" id="compile" href="#" title="Compile Code" onclick="javascript: compile(editor.getValue());"></span>Compile & Run</span> </a>');
...
{ 
	if(act == "back"){		
	...
	}else if (act == "endModuleInMid"){
		//alert('endModuleInMid' + $('#qID').val());
		
		var t = $('#qID').val();
		$('#lastSid').val(t);
		flagEndModule = 1;
		$('#endModuleInMid').val(1);
		$('#reviewCountID').html($('#totalReviewCount').val());
		$("#sdropIDReview").toggle();
		$('#divSubmitModule').show();
		return false;
		
	}else {
		//alert($('#lastSid').val() +' = ' + sid);
		//alert('in else1');
		if($('#lastSid').val() == sid){
			//
		...
		// disable cancel button on popup
		//$('#moduleSubmissionPopupCancel').attr('onclick','').unbind('click');
		//$('#moduleSubmissionPopupOK').attr('onclick','').unbind('click');
		$('#moduleSubmissionDiv').html('Submitting... Please wait<br /><em>This may take upto 20 seconds</em>');
	...
	$('#pageLoadingDiv').show();
	//autoSaveTime = 600000;
	//setTimeout("autoSaveCode(editor.getValue())", 600000);	
	document.frmMain.submitAnswer.value=1;
	//document.frmMain.isSubmit.value=1;	
	
	document.frmMain.rdoAnswer.value= srcCode; 
	document.frmMain.rdoAnswerTmp.value= srcCode; 
					...
					$.ajax({
						...
						success: function(msg){
							...
							if(parseInt(msg) == 1) {
								...
								alert('You have entered an incorrect Set Number. Kindly check the correct Set Number on the hardcopy provided and re-enter the same.');
								flag=false;	
							}
						}
...
function showTestHelpPopup() {
	...
	$('#helpSpanID').html('Test Instructions');
	$("#helpContentDiv").load('help.html');
	$('#divHelp').show();	
...
function showDisclaimerPopup(companyID) {
	...
	$.ajax({
		...
		success: function(msg){
			//alert(msg);
			$('#helpSpanID').html('Terms and Conditions');
			$("#helpContentDiv").html(msg);
			$('#divHelp').show();	
		}
	});
...
function showModuleSpecificHelpPopup(moduleID) {
	//alert(moduleID);
	if(moduleID == 259 || moduleID == 260 || moduleID == 637) {
		//var moduleDirectionContent = $('#moduleDirectionContent').val();
		...
		*/
  } else if(moduleID == 260) {
		//var moduleDirectionContent = $('#moduleDirectionContent').val();
		$("#helpContentDiv").load('amazonHelp_FIX.html');//(moduleDirectionContent);
		//var amcatID = $('#amcatID').val();
		//$('#helpAmcatID').html(amcatID);
